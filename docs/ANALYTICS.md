...
| `gesture_performed` | 제스처 발생 시 | `gesture_type`(shake/roll/press/rub/stretch), `duration_ms`, (stretch만) `peak_stretch` | P1 |
...
- 화면 표시 시 home_viewed
- 제스처(흔들기/굴리기/누르기/문지르기/늘리기) 발생 시 gesture_performed
  (속성: gesture_type, duration_ms, 늘리기는 peak_stretch 추가)
